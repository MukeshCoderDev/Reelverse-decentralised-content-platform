...
export class NetworkService {
  ...
  /**
   * Add custom network configuration (for future extensibility)
   */
  addNetworkConfig(config: NetworkConfig): boolean {
    if (!this.validateNetworkConfig(config)) {
      return false;
    }
  ...
  /**
   * Remove network configuration (for future extensibility)
   */
  removeNetworkConfig(chainId: number): boolean {
    if (chainId === DEFAULT_CHAIN_ID) {
      return false; // Cannot remove default network
    }

    delete this.networkConfigs[chainId];
    return true;
  }
